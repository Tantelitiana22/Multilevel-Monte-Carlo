...
var_mean V_L_mean_lookback(int l,int n){
	...
	double buff1(0); //variable temporaire
		...
		for(int i=0;i<n;i++){
			temp=S_T_loock_back(l);
			buff=exp(-r)*((temp.c-temp.a)-(temp.d-temp.b));
			s+=buff;
			v+=buff*buff;
			buff1=exp(-r)*(temp.c-temp.a);
			s1+=buff1;
			v1+=buff1*buff1;
 ...
 * ********************************************************************/
 ...
 unsigned long long  N_l_lookback(int l,int L,double tab[] ){
...
double Y_l_lookback(int l, unsigned long long n){
	
    S_return temp;
    double buff(0);
	 if(l==0){
		for(int i=0;i<n;i++){
			temp=S_T_loock_back(0);
			buff+=exp(-r)*(temp.c-temp.a);
		}
