...
int  main(void){
	...
	var_mean valeur;
	//Fichier pour enregistrer les résultat
     ...
     ofstream mon_fichier4("nombre_l5.dat");
    ...
    do{    
		//Calcul de VL pour N=10000
		    valeur=V_L_mean_asiat(L,N);
			VL[L]=valeur.var;
			
			//Calcul des N_l 
			  for(int i=0;i<=L;i++){
				n[i]=N_l_asiat(i,L,VL);
		...
		// ICi on verifie si la condition N_l>N après on recalibre si c'est le cas
					   for(int i=0;i<=L;i++){
							if(n[i]>N){
								valeur=V_L_mean_asiat(i,n[i]);
								VL[i]=valeur.var;
								VLPl[i]=valeur.varPl;
								mean[i]=valeur.mean;
								meanPl[i]=valeur.meanPl;
								y_val[i]=Y_l_asiat(i,n[i]);
							}
						}
