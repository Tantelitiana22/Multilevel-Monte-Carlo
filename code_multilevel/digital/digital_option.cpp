...
int  main(void){
    ...
    do{    
		//Calcul de VL pour N=10000
		    valeur=V_L_mean_digital(L,N);
			VL[L]=valeur.var;
			
			  for(int i=0;i<=L;i++){
				n[i]=N_l_digital(i,L,VL);
	...
	// ICi on verifie si la condition N_l>N après on recalibre si c'est le cas
					   ...
					   for(int i=0;i<=L;i++){
							if(n[i]>N){
								valeur=V_L_mean_digital(i,n[i]);
								VL[i]=valeur.var;
								VLPl[i]=valeur.varPl;
								mean[i]=valeur.mean;
								meanPl[i]=valeur.meanPl;
								y_val[i]=Y_l_digital(i,n[i]);
							}
						}
  ...
  cout<<"eps="<<eps<<";"<<" le cout de calcul pour MC : eps^2*cost="<<(eps)*(eps)*comp_monte_carlo<<endl;
  cout<<"prix= "<<prix<<endl;
	return 0;
