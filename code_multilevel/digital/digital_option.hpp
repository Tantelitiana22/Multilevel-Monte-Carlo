...
/******************************************************************
 * une fonction indicatrice pour le payoff de l'option
 * /**************************************************************/
double H(double x,double k){
...
var_mean V_L_mean_digital(int l,int n){
	...
	double buff1(0); //variable temporaire
		...
		for(int i=0;i<n;i++){
			temp=S_T(l);
			buff=exp(-r)*(H(temp.a,K)-H(temp.b,K));
			s+=buff;
			v+=buff*buff;
			buff1=exp(-r)*(H(temp.a,K));
			s1+=buff1;
			v1+=buff1*buff1;
 ...
 * ********************************************************************/
 ...
 unsigned long long  N_l_digital(int l,int L,double tab[] ){
...
double Y_l_digital(int l, unsigned long long n){
